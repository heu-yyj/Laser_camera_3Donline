# -*- coding: utf-8 -*-
"""
激光 + ZMQ位姿 + ZeroMQ 实时数据发布（全系统单位：毫米 mm）
（修改版：位姿数据通过 ZMQ 接收，双向缓存匹配）
v8 版本 更改了marker的位置，新的形心相较于原形心x(+50mm) y(不变) z(-7mm)
"""
...
UDP_PORT = 8888
# ZMQ位姿数据接收配置
ZMQ_POSE_PORT = 5556  # 接收位姿数据的ZMQ端口
ZMQ_POSE_IPS = ["192.168.5.105"]  # 接收位姿数据的IP地址列表
...
T_cam_in_auv = np.array([389.0 - 50.0, 39.8, 405.0 + 7.0])  # 相机在AUV坐标系中的位置 (mm) - v8版本修正
...
def process_matched_frame(laser_ts_ns, laser_points, pose_ts_ms, auv_pos_mm, auv_quat, sync_delay_ms, laser_frame_num, pose_frame_num):
    ...
    print(f"[Published] Frame {frame_idx:05d} | {len(laser_points)} pts | sync_delay: {sync_delay_ms:+.1f}ms | 成功匹配 发送结果: {success_count}/{total_count} 个服务端成功")
...
def zmq_pose_thread():
    global first_pose_received, pose_frame_counter
    # 创建ZMQ接收socket
    pose_context = zmq.Context()
    pose_socket = pose_context.socket(zmq.SUB)
    
    # 连接到多个IP地址
    for ip in ZMQ_POSE_IPS:
        connect_addr = f"tcp://{ip}:{ZMQ_POSE_PORT}"
        pose_socket.connect(connect_addr)
        print(f"[ZMQ_POSE] 已连接到位姿数据源 {connect_addr}")
    
    pose_socket.setsockopt_string(zmq.SUBSCRIBE, "")  # 订阅所有消息
    print(f"[ZMQ_POSE] 已连接到位姿数据源，等待数据...")
    
    while running:
        try:
            # 接收位姿数据
            pose_data = pose_socket.recv_json(flags=zmq.NOBLOCK)
            
            # 解析位姿数据 (假设格式为 {"position": [x, y, z], "orientation": [qx, qy, qz, qw], "timestamp": timestamp})
            pos_mm = np.array(pose_data["position"])
            quat = np.array(pose_data["orientation"])
            ts_ms = pose_data["timestamp"]  # 假设时间戳单位为毫秒
            ...
            if not first_pose_received:
                ...
                print(f"[ZMQ_POSE] 收到第一帧位姿，时间戳: {ts_ms} ms")
...
if __name__ == "__main__":
    ...
    # 启动ZMQ位姿接收线程，替代原来的nokov线程
    threading.Thread(target=zmq_pose_thread, daemon=True).start()
